...
public class DB_MENU extends JFrame {
	...
	public DB_MENU() {
		...
		DB_UA ua = new DB_UA();
		 ...
		 JScrollPane pane = TABLE_MAKER.Make_UA_TABLE((DB_CONNECT)ua,panel,Table_N,"normal",disregard,disregard,disregard,disregard);
		...
		JCheckBox chckbxNewCheckBox_1 = new JCheckBox("Search by UserAgent");
		...
		JCheckBox chckbxNewCheckBox_3 = new JCheckBox("Order by UserAgent");
				...
				if (to_search.length() != 0 && search_ != " " ) {
					...
					if(search_ == "id" ) {
						if(to_search.replaceAll("\\D", "@") == to_search) { //|\\d+\\D+|\\D+\\d+|\\D+\\d+\\D+
							...
							DB.DB_UA ua = new DB.DB_UA();
							
				pane_search = TABLE_MAKER.Make_UA_TABLE((DB_CONNECT)ua,panel, Table_N,"search", to_search, search_, disregard, disregard);
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (order_ != "") {
					DB_UA ua = new DB_UA();
				pane_ordered = TABLE_MAKER.Make_UA_TABLE((DB_CONNECT)ua,panel, Table_N,"order", disregard, disregard, order_ , ORDER_STATE); //[ ASC | DESC ]
		...
		btnNewButton_2.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				panel.revalidate();
				panel.repaint(getBounds());
				Table_N.setText("USERAGENTS@VIEW");
				panel.add(pane);
