...
public class Generator extends Tls implements Callable<Integer> {
...
public Generator(int k, String[] url_list, String params, String keyWord,
...
public int run(int k, String[] Url_list , String Params , String KeyWord , ArrayList<Map<String,ArrayList<String>>> toWrite,
		ArrayList<HashMap<String, String>> agents, DB_RESULT db_res )  {
    //Unpack
    //#########################
    ...
    //#########################
       ...
       tls.secondcontext = tls.browser.newContext(new Browser.NewContextOptions()
        ...
        l.log(Logger.GeneratorStartedLOG  + Url_list[k+1]);
        if(Utils.isRobotsParsable(Url_list[k+1], user_agent)) {
            l.log(Url_list[k+1] + "Entered in can_fetch");
            tls.second_page = tls.secondcontext.newPage();
            tls.second_page.navigate(Url_list[k+1]);
            tls.second_page.setDefaultTimeout(5000);
            if(Cookies.ACCEPT_COOKIE_POPUP_NOASYNC(tls.second_page, Url_list[k+1])) {
            	l.log("Went past cookie popup for " + kstr);
                ...
                try {
                    ...
                    Object isDiscardedNULL = new Object();
                   try {
                    ArrayList<String> isDiscarded = new ArrayList<String>(Parsers.ParserNoAsync(tls.second_page, Params, KeyWord, Url_list[k+1]));
                    Map<String,ArrayList<String>> Entry = new HashMap<String,ArrayList<String>>();
                    ...
                    Entry.put(Url_list[k+1], isDiscarded);
                    toWrite.add(Entry);
                   }
                   catch(Exception e) {
                  	 isDiscardedNULL = null;
                    ...
                    if (isDiscardedNULL == null)
                        l.log(Logger.GeneratorIsDiscLOG + Url_list[k+1]);
                    ...
                    l.log("Done" + Url_list[k+1]);
                 
                }
                tls.second_page.close();
            }
            ...
            l.log(Logger.GeneratorNotAllowedOnPageLOG + Url_list[k+1]);}

              
        tls.secondcontext.close();
...
public Integer call() throws Exception {
	try {
	return run(k, Url_list , Params , KeyWord ,toWrite,agents,db_res);
	}
	catch(Exception e) {
		//l.log(e.toString());
		String kstr = Integer.toString(k);
