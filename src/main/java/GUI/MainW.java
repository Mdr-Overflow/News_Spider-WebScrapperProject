		...
		SwingWorker<Boolean, Integer> bar_worker = new SwingWorker<Boolean, Integer>() {
			@Override
			 protected Boolean doInBackground() throws Exception {
				futures.add((Future<Runnable>) fill())  ;{ return true; }
			   }
	   	...
	   	SwingWorker<Boolean, Integer> worker = new SwingWorker<Boolean, Integer>() {
			   @Override
			   protected Boolean doInBackground() throws Exception {
				  futures.add((Future<Runnable>) Proj.driver2.RunInGui(params, KeyWord, Real_URLS, rate, threads,Agents, options));{ return true; }
			   }
